...
RSpec.describe Kumiki::Components::FormSelect, type: :model do
  # Test initialization with default values
  it "initialize with defaults" do
    select = Kumiki::Components::FormSelect.new
  ...
  it "initialize rejects multiple parameter" do
    ...
    expect {
      Kumiki::Components::FormSelect.new(multiple: true)
    }.to raise_error(ArgumentError)

    expect {
      Kumiki::Components::FormSelect.new(multiple: false)
    }.to raise_error(ArgumentError)
  end
  ...
  it "html attributes with disabled state" do
    select = Kumiki::Components::FormSelect.new(disabled: true)
    expected = {
      class: "select select-bordered",
      disabled: true
    }
  ...
  it "html attributes with required state" do
    select = Kumiki::Components::FormSelect.new(required: true)
    expected = {
      class: "select select-bordered",
      required: true,
      "aria-required": true
  ...
  it "html attributes with error and required" do
    select = Kumiki::Components::FormSelect.new(error: true, required: true)
    expected = {
      class: "select select-bordered select-error",
      required: true,
      "aria-required": true,
  ...
  it "options with array of arrays" do
    ...
    select = Kumiki::Components::FormSelect.new(options: options)
    expect(select.options).to eq(options)
  end
  ...
  it "options with empty array" do
    select = Kumiki::Components::FormSelect.new(options: [])
    expect(select.options).to eq([])
  end
  ...
  it "various size values" do
    %w[xs sm lg].each do |size|
      select = Kumiki::Components::FormSelect.new(size: size)
  ...
  it "prompt attribute stored correctly" do
    select = Kumiki::Components::FormSelect.new(prompt: "Select an option")
    expect(select.prompt).to eq("Select an option")
  end

  it "prompt with nil value" do
    select = Kumiki::Components::FormSelect.new(prompt: nil)
    expect(select.prompt).to be_nil
  end

  it "prompt with empty string" do
    select = Kumiki::Components::FormSelect.new(prompt: "")
