...
RSpec.describe Kumiki::Components::FormError, type: :model do
  # Test initialization with default values
  it "initialize with defaults" do
    error = Kumiki::Components::FormError.new
  ...
  it "initialize with message" do
    error = Kumiki::Components::FormError.new(message: "This field is required")
  ...
  it "initialize with messages array" do
    ...
    error = Kumiki::Components::FormError.new(messages: messages)
  ...
  it "html attributes with html options" do
    ...
    error = Kumiki::Components::FormError.new(html_options: html_options)
  ...
  it "has errors with messages array" do
    error = Kumiki::Components::FormError.new(messages: [ "Error 1", "Error 2" ])
    expect(error.has_errors?).to eq(true)
  end
  ...
  it "has errors with empty message" do
    error = Kumiki::Components::FormError.new(message: "")
    expect(error.has_errors?).to eq(false)
  end

  it "has errors with nil message" do
    error = Kumiki::Components::FormError.new(message: nil)
    expect(error.has_errors?).to eq(false)
  end

  it "has errors with empty messages array" do
    error = Kumiki::Components::FormError.new(messages: [])
    expect(error.has_errors?).to eq(false)
  end

  it "has errors with nil messages array" do
    error = Kumiki::Components::FormError.new(messages: nil)
    expect(error.has_errors?).to eq(false)
  end
  ...
  it "error messages with object but no attribute" do
    ...
    error = Kumiki::Components::FormError.new(object: mock_object)
    expect(error.error_messages).to eq([])
  end

  it "error messages with attribute but no object" do
    error = Kumiki::Components::FormError.new(attribute: :email)
    expect(error.error_messages).to eq([])
  end
  ...
  it "default accessibility attributes" do
    error = Kumiki::Components::FormError.new(message: "Error message")
  ...
  it "error messages with html content" do
    ...
    error = Kumiki::Components::FormError.new(message: html_message)
  ...
  it "error messages with special characters" do
    ...
    error = Kumiki::Components::FormError.new(message: special_message)
  ...
  it "multiple field errors" do
    ...
    email_error = Kumiki::Components::FormError.new(
      object: mock_object,
      attribute: :email
    )

    password_error = Kumiki::Components::FormError.new(
      object: mock_object,
      attribute: :password
    )
