...
module Kumiki
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs Kumiki and generates configuration files"

      class_option :form_builder, type: :boolean, default: false,
                   desc: "Set Kumiki form builder as default"
      ...
      def create_initializer
        template "initializer.rb", "config/initializers/kumiki.rb"
      end

      def configure_form_builder
        return unless options[:form_builder]

        inject_into_file "config/initializers/kumiki.rb",
                         after: "Kumiki.configure do |config|\n" do
      ...
      def update_tailwind_config
        ...
        if tailwind_config_path
          ...
          say "\nPlease manually add Kumiki paths to your Tailwind content array:", :yellow
          say "  './app/components/kumiki/**/*.{rb,erb,html}'", :yellow
          say "  './app/javascript/kumiki/**/*.js'", :yellow
        end
      end
      ...
      def add_importmap_pins
        ...
        unless importmap_content.include?("controllers/kumiki")
          ...
          say "Added Kumiki controller pins to importmap", :green
        else
          say "Importmap already contains Kumiki pins", :yellow
        end
      end
      ...
      def update_stimulus_application
        ...
        # Check if kumiki controllers are already imported
        if application_js.include?("kumiki")
          say "Stimulus application already includes Kumiki controllers", :yellow
          return
        end

        # Add import for kumiki controllers
        import_statement = "\n// Import Kumiki Stimulus controllers\nimport \"controllers/kumiki\"\n"
        ...
        say "Updated app/javascript/application.js with Kumiki controllers", :green
      end
      ...
      def inject_engine_paths_to_tailwind(config_path)
        ...
        if config_content.include?("kumiki")
          say "Tailwind config already includes Kumiki paths", :yellow
          return
        end
        ...
        if config_content =~ /content:\s*\[/
          ...
          say "Updated #{config_path} to include Kumiki paths", :green
        else
      ...
      def show_post_install_message
        say "\n"
        say "=" * 80, :green
        say "Kumiki has been installed successfully!", :green
        say "=" * 80, :green
        say "\n"

        # Summary of what was installed
        say "Installation Summary:", :cyan
        say "  ✓ Created config/initializers/kumiki.rb"
        say "  ✓ Copied DaisyUI plugin to app/assets/javascripts/daisyui.js"
        say "  ✓ Updated importmap.rb with Kumiki controller pins"
        ...
        if options[:form_builder]
          ...
          say "    <%= form_with model: @user, builder: Kumiki::ApplicationFormBuilder do |f| %>"
          say "      <%= f.text_field :name %>"
          say "    <% end %>"
          say "\n"
          say "  Option 2 - Enable as default in config/initializers/kumiki.rb:"
        ...
        say "  GitHub: https://github.com/darinhaener/kumiki"
        ...
        say "=" * 80, :green
        say "Happy building with Kumiki!", :green
        say "=" * 80, :green
        say "\n"
      end
    end
