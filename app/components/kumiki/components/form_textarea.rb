module Kumiki
  module Components
    class FormTextarea
      attr_reader :name, :value, :placeholder, :rows, :cols, :size, :disabled,
      ...
      def css_classes
        ...
        # Add error class
        classes << "textarea-error" if error?
      ...
      def html_attributes
        ...
        attributes[:rows] = rows if rows
        attributes[:cols] = cols if cols
        ...
        html_options.each do |key, value|
          unless [ :class, "class", :name, :placeholder, :rows, :cols, :disabled, :readonly, :required ].include?(key)
